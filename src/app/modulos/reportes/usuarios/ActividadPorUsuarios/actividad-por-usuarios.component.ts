...
export class ActividadPorUsuariosComponent implements OnInit {
  ...
  obtenerUsuariosFiltrados(table: Table): void {
    this.usuariosFiltrados = table.filteredValue != null ? table.filteredValue : this.logsDeUsuario;
  }
    ...
    import("jspdf").then(jsPDF => {
      import("jspdf-autotable").then(x => {
          ...
          function agregarTabla(columnas, filas, titulo,startY) {
            ...
            doc.text(titulo, 30, startY - 5);
    
          doc.autoTable({
            head: [columnas],
            body: filas,
            startY: startY ,
            margin: { top:60, right: 30,  left: 30 },
        ...
        agregarTabla(
          ["Dirección mac", "Fecha", "Acción", "Descripción"],
          this.logsDeUsuario.map(usu => [
            usu.macc,
            new Date(usu.fecha).toLocaleDateString(),
            usu.logAccion,
            usu.descripcion
          ]),
          "Listado de las actividades",300
        );    
