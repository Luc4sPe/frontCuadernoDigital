...
export class AgroquiMasUsadosPorProductorComponent {
  ...
  agroquimicosFiltrados: AplicacionAgroquimico[] = [];
 ...
 rows:any[]=[];
  ...
  obtenerApliAgroFiltrados(table: Table): void {
    this.agroquimicosFiltrados = table.filteredValue != null ? table.filteredValue : this.apliAgroquimico;
  }
  ...
  exportarPdf(table : Table){
    this.obtenerApliAgroFiltrados(table);
   
    const DATA =document.getElementById('grafico1');
    const options = {
      background: 'white',
      scale: 3
    };
    // configuracion de la tabla
      ...
      var columns = [
        ...
        { title: "Dosis por hectarea", dataKey: "dosisPorHectaria" },
        { title: "Objetivo", dataKey: "objetivo" },
        { title: "Aplicación", dataKey: "fechaDeAplicacion" }
      ];
    
      var rows = [];
    
      this.apliAgroquimico.map(function (x) {
        var fechaDeAplicacion = new Date(x.fechaDeAplicacion);
        rows.push({
          agroquimico: x.agroquimico.nombreComercial,
          cuadro: x.cuadro.numeroCuadro,
          dosisPorHectaria: x.dosisPorHectaria,
          objetivo: x.objetivo,
          fechaDeAplicacion: fechaDeAplicacion.toLocaleDateString()
        });
      });
    
    
    
    // Uso de la función en la configuración de autoTable
   /*  const tableData = generateTableData();
    
    var tableConfig = {
      head: [tableData.columns.map(column => column.title)],
      body: tableData.rows.map(row => tableData.columns.map(column => row[column.dataKey])),
      startY: 50,
      startX: 20
    }; */
    ...
    import("jspdf").then(jsPDF => {
      import("jspdf-autotable").then(x => {
          ...
          doc.autoTable( columns,rows,{startY: 300 ,didDrawPage: function(columna){
          ...
          doc.save('agroquimicosMasUsados.pdf');
