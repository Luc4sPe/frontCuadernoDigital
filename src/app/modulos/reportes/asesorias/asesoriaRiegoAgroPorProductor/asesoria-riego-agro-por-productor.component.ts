import { Component, OnInit } from '@angular/core';
import { AsesoriaAgroquimico } from 'src/app/Core/modelo/asesoria-agroquimico';
import { AsesoriaRiego } from 'src/app/Core/modelo/asesoria-riego';
import { AsesoriaAgroquimicoService } from 'src/app/service/asesoria-agroquimico.service';
import { AsesoriaRiegoService } from 'src/app/service/asesoria-riego.service';
import { Table } from 'primeng/table';
import html2canvas from 'html2canvas';
import { FechaDesdeHastaService } from 'src/app/service/fecha-desde-hasta-service';
import Swal from 'sweetalert2';
@Component({
  selector: 'app-asesoria-riego-agro-por-productor',
  templateUrl: './asesoria-riego-agro-por-productor.component.html',
  styleUrls: ['./asesoria-riego-agro-por-productor.component.css']
})
export class AsesoriaRiegoAgroPorProductorComponent implements OnInit {

  datos : any;
  options : any; 
  nombre: string = "";
  paletaDeColores = ['#003f5c','#2f4b7c', '#665191','#a05195', '#d45087', '#f95d6a', '#ff7c43', '#ffa600','#1c9be8', '#00b4eb', '#00c8cd', '#00d696', '#85dc56', '#e8d71f']
  cardSubtitulo : string = '';
  cantidadAsesoriaRiegoAplicadaByProductor: number = 0;
  cantidadAsesoriaRiegoNoAplicadaByProductor: number = 0;
  cantidadAsesoriaAgroAplicadaByProductor: number = 0;
  cantidadAsesoriaAgroNoAplicadaByProductor: number = 0;
  subtituloGrafico : string = 'Para visualizar el grafico, debes ingresar las fechas y nombre de usuario';
  tituloGrafico : string = 'Asesorías';
  asesoriaRiego: AsesoriaRiego []=[];
  loading : boolean = true;
  tituloTabla : string = '';
  subTituloTabla: string ='';
  rangoFechas : Date[] = [new Date(), new Date()];

  asesoriaAgro: AsesoriaAgroquimico []=[];
   //optengo fecha y hora actual del sistema
  fechaHoy: Date = new Date();


  constructor(
    private asesoriaRiegoService : AsesoriaRiegoService,
    private asesoriaAgroService : AsesoriaAgroquimicoService,
    private fechaDesdeHastaService : FechaDesdeHastaService,    
  ) { }
  ngOnInit(): void {
  
       this.rangoFechas = this.fechaDesdeHastaService.getFechaDesdeHasta(this.rangoFechas[0], this.rangoFechas[1]);
      /* this.listadoAsesoriaAgriquimico();
      this.listadoAsesoriaRiego(); */
     
  }

  /* cargarDatos(): void{

    this.tituloGrafico = `Aplicaciones de ${this.nombre}`;
    this.subtituloGrafico = 'Grafico'

    this.asesoriaRiegoService.cantidadAsesoriaRiegoAplicadaByProductor(this.nombre).toPromise().then(data =>{
      this.cantidadAsesoriaRiegoAplicadaByProductor = data;
      this.loading = false;
      this.cargarGrafico();
      this.listadoAsesoriaRiego();
   }),

   this.asesoriaRiegoService.cantidadAsesoriaRiegoNoAplicadaByProductor(this.nombre).toPromise().then(data =>{
    this.cantidadAsesoriaRiegoNoAplicadaByProductor = data;
    this.loading = false;
    this.cargarGrafico();
    this.listadoAsesoriaRiego();
 }),
  this.asesoriaAgroService.cantidadAsesoriaAgroAplicadaByProductor(this.nombre).toPromise().then(data =>{
    this.cantidadAsesoriaAgroAplicadaByProductor = data;
    this.loading = false;
    this.cargarGrafico();
    this.listadoAsesoriaRiego();
  }),

  this.asesoriaAgroService.cantidadAsesoriaAgroNoAplicadaByProductor(this.nombre).toPromise().then(data =>{
    this.cantidadAsesoriaAgroNoAplicadaByProductor = data;
    this.loading = false;
    this.cargarGrafico();
    this.listadoAsesoriaRiego();
    this.listadoAsesoriaAgriquimico();
  })
 */
  

 // }
 listadoAsesoriaRiego(): void{
  this.asesoriaRiegoService.listarAsesoriaPorProductor(this.nombre).subscribe(data =>{
    this.tituloTabla="Listado asesorías de riego"
    this.subTituloTabla ='Productor '+this.nombre;
    this.asesoriaRiego=data;
    this.asesoriaRiego.forEach(aseRi => {new Date (aseRi.fechaAplicacionAsesoria)});
    this.loading = false;
  })
 
}

listadoAsesoriaAgriquimico(): void{
  this.asesoriaAgroService.listarAsesoriAgroPorProductor(this.nombre).subscribe(data =>{
    this.tituloTabla='Listado de asesorías de agroquímico';
    this.subTituloTabla ='Productor '+this.nombre;
    this.asesoriaAgro=data;
    this.asesoriaAgro.forEach(aseAgr => { new Date(aseAgr.fechaAsesoriaAgroquimico)});
    this.loading = false;
   
  })
 
}

  datosFiltrados():void{
   
    this.subtituloGrafico = 'Grafico';
    this.rangoFechas = this.fechaDesdeHastaService.getFechaDesdeHasta(this.rangoFechas[0], this.rangoFechas[1]);
    const fechaDesde = this.rangoFechas[0];
    const fechaHasta = this.rangoFechas[1];  
    this.subtituloGrafico = `Desde ${fechaDesde.toLocaleDateString()} hasta ${fechaHasta.toLocaleDateString()}.`
    this.tituloGrafico = `Asesorías de ${this.nombre}`;
    this.asesoriaRiegoService.listarAsesoriaPorProductor(this.nombre).subscribe(
      data =>{
        
        this.asesoriaRiego = data.filter(aseRiego => new Date(aseRiego.fechaAsesoriaRiego) >= fechaDesde && new Date(aseRiego.fechaAsesoriaRiego) <= fechaHasta);
        this.loading=false;
      },err => {
        Swal.fire('Error, nombre de usuario incorrecto', err.error.message, 'error')
      }
    );

    this.asesoriaAgroService.listarAsesoriAgroPorProductor(this.nombre).subscribe(
      data =>{
        this.asesoriaAgro = data.filter(aseAgo => new Date(aseAgo.fechaAsesoriaAgroquimico) >= fechaDesde && new Date(aseAgo.fechaAsesoriaAgroquimico) <= fechaHasta);
        this.loading=false;
      },err => {
        Swal.fire('Error, nombre de usuario incorrecto', err.error.message, 'error')
      }
    );
    this.cargarGrafico();

  }

   cargarGrafico(): void {
    //grafico
 
    const aseriego = this.asesoriaRiego.map(aseR => aseR.asesoriaAplicada);
    const aseAgro = this.asesoriaAgro.map(aseAgro => aseAgro.asesoriaAplicada);
  
    // Contar aplicaciones verdaderas
  const apliVerRiego = aseriego.filter(aplicada => aplicada === true).length;
  const apliVerAgro = aseAgro.filter(aplicada => aplicada === true).length;
  
  // Contar aplicaciones falsas
  const aplifalsRiego = aseriego.filter(aplicada => aplicada === false).length;
  const apliFalseAgro =  aseAgro.filter(aplicada => aplicada ===false).length;

    this.datos = {
      labels: ['Aplicada', 'No aplicada'],
      datasets : [
        {
          label : 'Asesoria riego',
          data :[apliVerRiego,aplifalsRiego],
          backgroundColor : this.paletaDeColores[1],
          borderColor : ['black'],
          borderWidth: 1 
          
        },
        {
          label : 'Asesoria agroquímico',
          data :[apliVerAgro, apliFalseAgro],
          backgroundColor : this.paletaDeColores[11],
          borderColor : ['black'],
          borderWidth: 1
          
        }
      ],
      
    }
    this.options = {
      indexAxis: 'x',
      maintainAspectRatio: false,
      aspectRatio: 0.8,
      
    }    
  }

 

  getSeverityByEstado(asesoriaAgro : AsesoriaAgroquimico): string {
    const serverityByEstado : {[key: string]: string} = {
      true : 'success',
      false: 'danger'
    };
    return serverityByEstado[`${asesoriaAgro.asesoriaAplicada}`];
  }

  clear(table : Table) {
    table.clear();
  }

  exportarPdf(){
    const DATA =document.getElementById('graficoAse');
    const options = {
      background: 'white',
      scale: 3
    };
 /*    
      // configuracion de la tabla asesoría agroquímico
   
      var columnaAgro = [
        { title: "Finca", dataKey: "finca" },
        { title: "Cuadro", dataKey: "cuadro" },
        { title: "Agroquímico", dataKey: "agroquimico" },
        { title: "Dosis/ha", dataKey: "dosisPorHectaria" },
        { title: "Dosis/Hl", dataKey: "dosisPorHl" },
        { title: "Vol./ha", dataKey: "volumenPorHectaria" },
        { title: "Objetivo", dataKey: "objetivo" },
        { title: "Plaga", dataKey: "plaga" },
        { title: "Fecha estimada", dataKey: "fechaEstimadaAplicacionParsed" },
        { title: "Fecha aplicación", dataKey: "fechaAplicacionAsesoria" },
        { title: "Encargado agrícola", dataKey: "nombreEncargado" }
      ];
    
      var filaRowsAg = [];
    
      this.asesoriaAgro.map(function (a) {
        var fechaEstimadaA = new Date(a.fechaEstimadaAplicacionParsed);
        var fechaDeAplicacionA = a.fechaAplicacionAsesoria ? new Date(a.fechaAplicacionAsesoria): null;
       
        filaRowsAg.push({
          finca: a.finca.nombre,
          cuadro: a.cuadro.numeroCuadro,
          agroquimico: a.agroquimico.nombreComercial,
          dosisPorHectaria: a.dosisPorHectaria,
          dosisPorHl:a.dosisPorHl,
          volumenPorHectaria: a.volumenPorHectaria,
          objetivo: a.objetivo,
          plaga:a.plaga,
          fechaEstimadaAplicacionParsed: fechaEstimadaA.toLocaleDateString(),
          fechaAplicacionAsesoria: fechaDeAplicacionA
          ? fechaDeAplicacionA.toLocaleDateString()
          : "No aplicada",
          nombreEncargado:a.nombreEncargado,
        });
      });
    
    //fin tabla asesoria */

    html2canvas(DATA,options).then((canvas) => {
    
    import("jspdf").then(jsPDF => {
      import("jspdf-autotable").then(x => {

          var doc : any = new jsPDF.default('p','pt');
          // exportar imagen grafico
          const img = canvas.toDataURL('image/PNG');
          // Add image Canvas to PDF
          const bufferX = 15;
          const bufferY = 70;
          const imgProps = (doc as any).getImageProperties(img);
          const pdfWidth = doc.internal.pageSize.getWidth() - 2 * bufferX;
          const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;  
                 
          var ima="data:image/png;base64,";
          
          doc.addImage(img, 'PNG', bufferX, bufferY, pdfWidth, pdfHeight, undefined, 'FAST');
          function agregarTabla(columnas, filas, titulo,startY) {

                 // Verifica si la tabla cabe en la página actual
              if (doc.autoTable.previous.finalY > 500 ){
                // Si no cabe, agrega una nueva página y establece startY en la parte superior
                doc.addPage();
                startY = 80; // Puedes ajustar este valor según tus necesidades
      
              } 
     
    
                doc.setFontSize(10);
                doc.setFontSize(10);
                doc.setFont("courier", "bolditalic");
                doc.text(titulo, 25, startY - 5 );
              
                doc.autoTable({
                head: [columnas],
                body: filas,
                startY: startY ,
                margin: { top: 10, right: 24,  left: 24 },
                didDrawPage: function(columnas){
                  doc.setFontSize(10);
                  doc.text("Página " + doc.internal.getNumberOfPages(), columnas.settings.margin.left, doc.internal.pageSize.height - 10);
                  doc.addImage(ima,280,15,50,25);
                  doc.setFontSize(10);
                  doc.text(`Fecha: ${new Date().toLocaleDateString()}`,480,30);
              }, columnStyles: {
                fechaEstimadaAplicacionParsed: { cellWidth: 100},
                fechaAplicacionAsesoria:{cellWidth:100},
                dosisPorHectaria:{cellWidth:25},
                dosisPorHl:{cellWidth:25},
                volumenPorHectaria:{cellWidth:25},
                
              }
              });
        }
           
        agregarTabla(
          ["Cuadro", "Duración", "Milimetros aplicados", "Fecha estimada", "Fecha aplicada"],
          this.asesoriaRiego.map(aseR => [
            aseR.cuadro.numeroCuadro,
            aseR.duracionEnHoras,
            aseR.milimetrosAplicados,
            new Date(aseR.fechaEstimadaAplicacionParsed).toLocaleDateString(),
            aseR.fechaAplicacionAsesoria ?  new Date(aseR.fechaAplicacionAsesoria).toLocaleDateString() : "No aplicada",
          ]),
          "Listado asesoría riego",380
        );

         agregarTabla(
          ["Agroquímico", "Cuadro", "Dosis/ha", "Dosis/Hl", "Vol./ha", "Objetivo", "Plaga", "Fecha estimada","Fecha aplicada", "Encargado"],
          this.asesoriaAgro.map(ase => [
            ase.agroquimico.nombreComercial,
            ase.cuadro.numeroCuadro,
            ase.dosisPorHectaria,
            ase.dosisPorHl,
            ase.volumenPorHectaria,
            ase.objetivo,
            ase.plaga,
            new Date(ase.fechaEstimadaAplicacionParsed).toLocaleDateString(),
            ase.fechaAplicacionAsesoria ? new Date(ase.fechaAplicacionAsesoria).toLocaleDateString() : "No aplicada",
            ase.nombreEncargado,
          ]),
          "Listado asesoría de agroquímico", doc.autoTable.previous.finalY + 50
          ); 
          
         
         /*  doc.autoTable( columns,rows,{startY: 350 ,didDrawPage: function(columna){
            doc.setFontSize(10);
            doc.setFont("courier", "bolditalic");
            doc.text(`Listado asesoría de riego`,40,340);
            doc.setFontSize(10);
            doc.text('Página ' + doc.internal.getNumberOfPages(), columna.settings.margin.left, doc.internal.pageSize.height - 10);
            doc.addImage(ima,280,10,50,25);
            doc.setFontSize(10);
            doc.text(`Fecha: ${new Date().toLocaleDateString()}`,480,30);
        }
            
      } );
        doc.autoTable(columnaAgro,filaRowsAg,{startY: doc.autoTable.previous.finalY + 320,theme: 'grid',didDrawPage: function(columna){
          doc.setFontSize(10); 
        doc.setFont("courier", "bolditalic");
         doc.text(`Listado asesoría de agroquímico`,40,30);
          doc.setFontSize(10);
          doc.text('Página ' + doc.internal.getNumberOfPages(), columna.settings.margin.left, doc.internal.pageSize.height - 10);
          doc.addImage(ima,280,10,50,25);
          doc.setFontSize(10);
          doc.text(`Fecha: ${new Date().toLocaleDateString()}`,480,30);
      },  columnStyles: {
        fechaEstimadaAplicacionParsed: { cellWidth: 60},
        fechaAplicacionAsesoria:{cellWidth:60},
        dosisPorHectaria:{cellWidth:40},
        dosisPorHl:{cellWidth:40},
        volumenPorHectaria:{cellWidth:35},
        
      }, margin: {  right: 10,  left: 10 }
    }); */
         
          doc.save('asesoriasMasUsadas.pdf');
        
      })
    }) 
  })
  }

 





}
