...
export class AsesoriaRiegoAgroquimicoComponent implements OnInit{
  ...
  asesoriasRiego: AsesoriaRiego[];
  asesoriasAgro: AsesoriaAgroquimico[];
  loading : boolean = true;
  subtituloGrafico : string = 'Para visualizar el grafico, debes seleccionar las fechas';
  tituloGrafico : string = 'Asesorías en general';


  constructor(
    private fechaDesdeHastaService : FechaDesdeHastaService,
    private asesoriaAgrService:AsesoriaAgroquimicoService,
    private asesoriaRieService:AsesoriaRiegoService
...
/*  cargarAlInicio(){
  ...
  obtenerAsesorias(): void{
 
  this.asesoriaRieService.listarAsesoria().subscribe(
    data =>{
      
      this.asesoriasRiego = data;
      this.loading=false;
      this.cargarGraficoSinFiltro();
    },
    err => {
  ...
  this.asesoriaAgrService.listarAsesoria().subscribe(
    data =>{
      this.asesoriasAgro=data;
      this.cargarGraficoSinFiltro();
      this.loading=false;
    },
    err => {
 ...
 cargarGrafico(): void {
  ...
  this.asesoriaRieService.listarAsesoria().subscribe(
    data =>{
      ...
      this.asesoriasRiego = data.filter(aseRiego => new Date(aseRiego.fechaAsesoriaRiego) >= fechaDesde && new Date(aseRiego.fechaAsesoriaRiego) <= fechaHasta);
      this.loading=false;
    },err => {
      Swal.fire('Error, asesorías de riego no encontradas', err.error.message, 'error')
    }
  );

  this.asesoriaAgrService.listarAsesoria().subscribe(
    data =>{
      this.asesoriasAgro=data.filter(aseAgo => new Date(aseAgo.fechaAsesoriaAgroquimico) >= fechaDesde && new Date(aseAgo.fechaAsesoriaAgroquimico) <= fechaHasta);
      this.loading=false;
    },err => {
      Swal.fire('Error, asesorías de agroquímocps no encontradas', err.error.message, 'error')
    }
  );

  //Recorro las asesoŕias.

  const aseriego = this.asesoriasRiego.map(aseR => aseR.asesoriaAplicada);
  const aseAgro = this.asesoriasAgro.map(aseAgro => aseAgro.asesoriaAplicada);
...
cargarGraficoSinFiltro(): void {
 
  this.subtituloGrafico = `Desde todos los tiempos`
 ...
 */
  ...
  exportarPdf(){
    //Obtenemos lo q queremos capturar
     const DATA =document.getElementById('pdfAgro');
     const options = {
       background: 'white',
       scale: 3
     };
     ...
     import("jspdf").then(jsPDF => {
       import("jspdf-autotable").then(x => {
           ...
           doc.autoTable({didDrawPage: function(exportColumns){
            ...
            //doc.text("Grafico de todas las asesorías  ", 25, 30 );
             doc.setFontSize(10);
             doc.text("Página " + exportColumns.pageNumber + " de " + doc.internal.getNumberOfPages(), exportColumns.settings.margin.left, doc.internal.pageSize.height - 10);
           ...
           //imprimir
          /*  doc.autoPrint(); */
           doc.save('asesoriasMasUsuadas.pdf');
